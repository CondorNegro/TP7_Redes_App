...
sudo docker exec bgp_h11_1 ip -6 route add default via 2001:aaaa:aaaa:6::5
sudo docker exec bgp_h13_1 ip -6 route add default via 2001:aaaa:aaaa:7::7

echo "### AGREGANDO GATEWAY ESTATICO ###"
sudo docker exec bgp_r3_1 ip -6 route add default via 2001:aaaa:aaaa:1::111
sudo docker exec bgp_r5_1 ip -6 route add default via 2001:aaaa:aaaa:2::111
sudo docker exec bgp_r7_1 ip -6 route add default via 2001:aaaa:aaaa:3::111
